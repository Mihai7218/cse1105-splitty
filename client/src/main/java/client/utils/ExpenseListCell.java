...
public class ExpenseListCell extends ListCell<Expense> {
    ...
    protected void updateItem(Expense item, boolean empty) {
        ...
        if (empty || item == null) {
            ...
            if(this.getItem().getDescription().equals("transfer")){
                createTransfer(item);
                updateTransfer();
            }else if(this.getItem().getDescription().equals("settlement")){
                createSettlement(item);
                updateSettlement();
            }else {
                createGraphic(item);
                update();
    ...
    private void createSettlement(Expense item) {
        ...
        payeeName = new Label();
        forLabel = new Label();
        forLabel.textProperty().bind(languageManager.bind("overview.withLabel"));
        price = new Label();
        currency = new Label();
        date = new Label();
        payers = new Label();
    ...
    private void updateSettlement(){
        ...
        tagName.textProperty().bind(languageManager.bind("overview.settledLabel"));
